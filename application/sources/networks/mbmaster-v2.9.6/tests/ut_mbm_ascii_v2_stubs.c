 ...
 * All rights reserved.
 *
 * $Id: ut_mbm_ascii_v2_stubs.c,v 1.2 2011-05-22 22:29:05 embedded-solutions.cwalter Exp $
 */
...
#define EXPECTED_REQUEST_BUFFER_SIZE    ( 520 )
#define PREPARED_RESPONSE_BUFFER_SIZE   ( 520 )
                ...
                UCHAR ucDataBits, eMBSerialParity eParity, UCHAR ucStopBits, xMBHandle xMBHdl )
{
    ...
    if( !bFailOnInit )
    {
        ...
        if( ( ucPort < MB_UTILS_NARRSIZE( xSerialHdl ) ) && ( IDX_INVALID == xSerialHdl[ucPort].ubIdx ) )
        {
            ...
            xSerialHdl[ucPort].xMBHdl = xMBHdl;
...
vMBMTestRequestTXThread( void *pvData )
{
    ...
    BOOL            bMoreTXData;
    UBYTE           ubExpectedBytes[TX_BUFFER_SIZE];
        ...
        if( isTxEnabled )
        {
            ...
            bMoreTXData =
                pxThreadData->pxSerialIntHdl->pbMBPTransmitterEmptyFN( pxThreadData->pxSerialIntHdl->xMBHdl,
                                                                       ubExpectedBytes, TX_BUFFER_SIZE,
                                                                       &usBytesWritten );
            if( !bMoreTXData )
            {
            ...
            {
                CU_ASSERT_TRUE( 0 ==
                                memcmp( &( pxThreadData->arubExpectedRequest[pxThreadData->usExpectedRequestCurPos] ),
                                        ubExpectedBytes, usBytesWritten ) );
                pxThreadData->usExpectedRequestCurPos += usBytesWritten;
            }
        }
    }
    while( isTxEnabled );
    ...
    else
    {
        ...
        do
        {
            ...
            if( isRxEnabled )
            {
                ...
                usRxBytesToSend = pxThreadData->usPreparedResponseLen - usRxBufPos;
                if( usRxBytesToSend > RX_BUFFER_SIZE )
                {
                    usRxBytesToSend = RX_BUFFER_SIZE;
                }
