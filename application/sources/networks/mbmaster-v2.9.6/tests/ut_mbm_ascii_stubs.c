 ...
 * All rights reserved.
 *
 * $Id: ut_mbm_ascii_stubs.c,v 1.10 2011-05-22 22:29:05 embedded-solutions.cwalter Exp $
 */
...
vMBMTestRequestTXThread( void *pvData )
{
    ...
    BOOL            bMoreTXData;
    UBYTE           ubExpectedCharacter, ubCompareTo;
    ...
    do
    {
        ...
        if( isTxEnabled )
        {
            ...
            {
                ubCompareTo = pxThreadData->arubExpectedRequest[pxThreadData->usExpectedRequestCurPos];
                if( ubExpectedCharacter != ubCompareTo )
                {
                    CU_ASSERT( 0 );
                }
