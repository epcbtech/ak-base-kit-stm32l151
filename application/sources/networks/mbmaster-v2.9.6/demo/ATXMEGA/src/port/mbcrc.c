/* 
 * MODBUS Library: ATxmega port
 * Copyright (c) 2014 Christian Walter <cwalter@embedded-solutions.at>
 * All rights reserved.
 *
 * $Id: mbcrc.c,v 1.1 2014-03-09 12:51:23 embedded-so.embedded-solutions.1 Exp $
 */

/* ----------------------- System includes ----------------------------------*/
#include <avr/pgmspace.h>

/* ----------------------- Platform includes --------------------------------*/
#include "mbport.h"

USHORT usMBMCRC16( UCHAR * pucFrame, USHORT usLen );
USHORT usMBSCRC16( UCHAR * pucFrame, USHORT usLen );

static const PROGMEM UCHAR aucCRCHi[] = {
    
};

static const PROGMEM UCHAR aucCRCLo[] = {
    
};


USHORT
usMBMCRC16( UCHAR * pucFrame, USHORT usLen )
{
    UCHAR           ucCRCHi = 0xFF;
    UCHAR           ucCRCLo = 0xFF;
    int             iIndex;

    while( usLen-- )
    {
        iIndex = ucCRCLo ^ *( pucFrame++ );
        ucCRCLo = ucCRCHi ^ pgm_read_byte( &aucCRCHi[iIndex] );
        ucCRCHi = pgm_read_byte( &aucCRCLo[iIndex] );;
    }
    return ucCRCHi << 8 | ucCRCLo;
}

USHORT
usMBSCRC16( UCHAR * pucFrame, USHORT usLen )
{
    return usMBMCRC16( pucFrame, usLen );
}
