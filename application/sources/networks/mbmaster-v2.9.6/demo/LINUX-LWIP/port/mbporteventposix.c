 ...
 * All rights reserved.
 *
 * $Id: mbporteventposix.c,v 1.1 2014-08-23 11:48:50 embedded-solutions.cwalter Exp $
 */
    ...
    if( NULL != pxEventHdl )
    {
        ...
        for( i = 0; i < MAX_EVENT_HDLS; i++ )
        {
            if( IDX_INVALID == arxEventHdls[i].ubIdx )
            {
                ...
                ( void )snprintf( arucMQName, MB_UTILS_NARRSIZE( arucMQName ), MESSAGE_QUEUE_NAME_FMT,
                                  ( int )getpid(  ), i );
                ...
                else
                {
                    ...
                    {
						vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_EVENT, "Can't create event queue '%s': %s", arucMQName,
									 strerror( errno ) );
                    }
#endif
                    eStatus = MB_EPORTERR;
...
eMBPEventPost( const xMBPEventHandle xEventHdl, xMBPEventType xEvent )
{
    ...
    if( MB_IS_VALID_HDL( pxEventHdl, arxEventHdls ) )
    {
        ...
        if( 0 != mq_send( xMQHdlTemp, ( char * )&xEvent, sizeof( xMBPEventType ), MESSAGE_PRIORITY ) )
        {
			...
			if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_EVENT ) )
			{
				vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_EVENT, "Can't post event in queue %d: %s", ( int )xMQHdlTemp,
							 strerror( errno ) );
			}
#endif
            eStatus = MB_EPORTERR;
...
bMBPEventGet( const xMBPEventHandle xEventHdl, xMBPEventType * pxEvent )
{
    ...
    if( MB_IS_VALID_HDL( pxEventHdl, arxEventHdls ) )
    {
        ...
        else
        {
			...
			if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_EVENT ) )
			{
				vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_EVENT, "Can't receive event from queue %d: %s", ( int )xMQHdlTemp,
							 strerror( errno ) );
			}
#endif
        }
...
vMBPEventDelete( xMBPEventHandle xEventHdl )
{
    ...
    if( MB_IS_VALID_HDL( pxEventIntHdl, arxEventHdls ) )
    {
        ...
        {
            ( void )snprintf( arucMQName, MB_UTILS_NARRSIZE( arucMQName ), MESSAGE_QUEUE_NAME_FMT, ( int )getpid(  ),
                              pxEventIntHdl->ubIdx );
