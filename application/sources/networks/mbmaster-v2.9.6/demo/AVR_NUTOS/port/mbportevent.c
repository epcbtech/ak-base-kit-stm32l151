 ...
 * All rights reserved.
 *
 * $Id: mbportevent.c,v 1.1 2010-02-21 19:23:07 embedded-so.embedded-solutions.1 Exp $
 */
...
eMBPEventPost( const xMBPEventHandle xEventHdl, xMBPEventType xEvent )
{
    ...
    if( MB_IS_VALID_HDL( pxEventHdl, arxEventHdls ) )
    {
        ...
        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_OTHER ) )
        {
            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_OTHER,
                         "[IDX=" MBP_FORMAT_USHORT "] Posted event " MBP_FORMAT_USHORT "\n", pxEventHdl->ubIdx,
                         ( USHORT ) xEvent );
        }
#endif
        pxEventHdl->xType = xEvent;
        ( void )NutEventPost( &pxEventHdl->xHdl );
        eStatus = MB_ENOERR;
    }
    MBP_EXIT_CRITICAL_SECTION(  );
    return eStatus;
...
bMBPEventGet( const xMBPEventHandle xEventHdl, xMBPEventType * pxEvent )
{
    ...
    if( MB_IS_VALID_HDL( pxEventHdl, arxEventHdls ) )
    {
        if( 0 == NutEventWait( &( pxEventHdl->xHdl ), 100 ) )
        {
            MBP_ENTER_CRITICAL_SECTION(  );
            ...
            if( bEventInQueue )
            {
                ...
                {
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_OTHER,
                                 "[IDX=" MBP_FORMAT_USHORT "] Received event " MBP_FORMAT_USHORT "\n",
                                 pxEventHdl->ubIdx, ( USHORT ) * pxEvent );
                }
            }
#endif
        }
