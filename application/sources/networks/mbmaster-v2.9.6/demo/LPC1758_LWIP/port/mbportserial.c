 ...
 * All rights reserved.
 *
 * $Id: mbportserial.c,v 1.2 2011-01-02 16:52:00 embedded-solutions.cwalter Exp $
 */
...
#define MBP_SERIAL_TASK_STACKSIZE           ( 96 )
#define MBP_SERIAL_BUFFER_SIZE	            ( 16 )
#define MBP_SERIAL_GUARDTIME_MS             ( 1 )
...
usMPSerialTimeout( ULONG ulBaudRate )
{
    ...
    /* Add some guard time */
    usTimeoutMS += MBP_SERIAL_GUARDTIME_MS;
    return usTimeoutMS;
}
...
vMBPSerialHandlerTask( void *pvArg )
{
        ...
        if( ( usEvents & DRV_SERIAL_EVENT_TXRDY ) > 0 )
        {
            ...
            if( MB_ENOERR == eStatus )
            {
                ...
                if( ( usByteMax > 0 ) && ( NULL != pxSerialIntHdl->pbMBPTransmitterEmptyFN ) )
                {
                    ...
                    if( !pxSerialIntHdl->pbMBPTransmitterEmptyFN
                        ( pxSerialIntHdl->xMBHdl, arubBuffer, usByteMax, &usCnt ) )
                    {
                        pxSerialIntHdl->pbMBPTransmitterEmptyFN = NULL;
                        MBP_EXIT_CRITICAL_SECTION(  );
    ...
    eStatus = eDrvSerialClose( pxSerialIntHdl->ubIdx );
    MBP_ASSERT( MB_ENOERR == eStatus );
