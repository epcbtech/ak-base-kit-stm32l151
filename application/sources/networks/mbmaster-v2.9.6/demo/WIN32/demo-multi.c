 ...
 * All rights reserved.
 *
 * $Id: demo-multi.c,v 1.1 2008-11-02 17:12:07 cwalter Exp $
 */
...
_tmain( int argc, _TCHAR * argv[] )
{
    ...
    else if( NULL == ( xThread1Hdl = CreateThread( NULL, 0, pvHandlerThread2, NULL, 0, &dwThread2Id ) ) )
    {
    }
    else
    {
...
pvHandlerThread1( LPVOID lpParameter )
{
    ...
    do
    {
            ...
            ( eStatus = eMBMSerialInit( &xMBMMaster, MB_RTU, MASTER1_PORT, MASTER1_BAUDRATE, MASTER1_PARITY ) ) )
        {
            ...
            if( MB_ENOERR !=
                ( eStatus2 =
                  eMBMReadHoldingRegisters( xMBMMaster, 1, 1, sizeof( usNRegs ) / sizeof( usNRegs[0] ), usNRegs ) ) )
            {
                eStatus = eStatus2;
            }
...
pvHandlerThread2( LPVOID lpParameter )
{
    ...
    {
        if( MB_ENOERR ==
            ( eStatus = eMBMSerialInit( &xMBMMaster, MB_RTU, MASTER2_PORT, MASTER2_BAUDRATE, MASTER2_PARITY ) ) )
        {
            /* Write an incrementing counter to register address 0. */
            if( MB_ENOERR != ( eStatus2 = eMBMWriteSingleRegister( xMBMMaster, 1, 0, usLoopCnt++ ) ) )
            {
