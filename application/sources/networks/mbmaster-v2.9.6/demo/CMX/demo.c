 ...
 * All rights reserved.
 *
 * $Id: demo.c,v 1.3 2008-09-01 18:44:49 cwalter Exp $
 */
    ...
    do
    {
        if( MB_ENOERR == ( eStatus = eMBMSerialInit( &xMBMMaster, MB_RTU, 0, 38400, MB_PAR_NONE ) ) )
        {
            do
            {
                ...
                /* Read holding register from adress 0 - 20 */
                if( MB_ENOERR != ( eStatus2 = eMBMReadHoldingRegisters( xMBMMaster, 1, 0, 20, usNRegs ) ) )
                {
                    eStatus = eStatus2;
                }
            }
